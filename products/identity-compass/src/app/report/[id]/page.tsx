...
import { Big5Bars, Gauge, StatusChips, ValueBars } from "@/components/ScoreBars";
import { hasFullAccess } from "@/lib/access";
import { buildReport, identityThesis } from "@/lib/engine/report";
...
export default async function ReportPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ paid?: string }> }) {
  ...
  const { data: rep } = await supabase.from("reports").select("narrative").eq("assessment_id", id).maybeSingle();
    ...
    <div className="container-prose py-10 space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="display mt-2 text-2xl text-accent-deep">«{thesis.line}»</p>
        </div>
        <PrintButton />
      </header>
      ...
      <Narrative assessmentId={id} existing={rep?.narrative ?? null} />
